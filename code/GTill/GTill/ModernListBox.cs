    ...
    public partial class ModernListBox : UserControl
    {
        ...
        void SmoothMoveBetweenTwoItems(int nCurrentItem, int nTargetItem)
        {
            if (bSmoothMoveEnabled)
            {
                ...
                {
                    lblItem[nCurrentItem].Font = new Font(GTill.Properties.Settings.Default.sFontName, lblItem[nCurrentItem].Font.Size - 3.0f);
                    lblItem[nTargetItem].Font = new Font(GTill.Properties.Settings.Default.sFontName, lblItem[nTargetItem].Font.Size + 3.0f);
                    RedrawItems();
                    this.Refresh();
                }
            }
