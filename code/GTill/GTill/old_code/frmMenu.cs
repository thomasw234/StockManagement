...
namespace GTill
{
    class frmMenu : Form
    {
        ...
        void fltTransToVoid_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (fltTransToVoid.TransactionNumber != "CANCELLED")
            {
                if (UserCertain("void transaction number " + fltTransToVoid.TransactionNumber))
                {
                    ...
                    tEngine.VoidTransaction(nToRemove, tEngine.GetCurrentStaffNumber());
                }
                else
                {
        ...
        {
            if (MessageBox.Show("Are you sure you want to " + sTask, "Till v2.0", MessageBoxButtons.YesNo) == DialogResult.Yes)
                return true;
            else
                return false;
        }
        ...
        void fltTransToRemove_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Are you sure you wish to remove transaction number " + sToRemove + "?", "Final Chance!", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
