...
namespace GTill
{
    class frmMain : Form
    {
        ...
        /// <summary>
        /// The amount of items to miss off the bottom (to allow scrolling up the display)
        /// </summary>
        int nDisplayMoveUpAmount = 0;
        /// <summary>
        ...
        public frmMain()
        {
            ...
            tillEngine.ChangeStaffNumber(0);
        ...
        void ChangeFormState(FormState stateToGoTo)
        {
            ...
            if (stateToGoTo == FormState.Loading)
            {
                ...
                mlbLoading = new ModernListBox.ModernListBox(29.0f, 14.0f, true);
                ...
                mlbLoading.Size = new Size(this.Width, 350);
        ...
        void LoadTill(bool bJustRedrawing)
        {
            ...
            lblTitle = new Label();
            lblTitle.Text = tillEngine.sShopName();
            ...
            lblHelp = new Label();
            lblHelp.Text = "(P = Reprint Receipt, C = Change Payment, L = Price Lookup + Stock Levels, D = Description Search)";
            if (!bJustRedrawing)
            ...
            lblTillID = new Label();
            lblTillID.Text = "Till = " + tillEngine.GetTillName();
            ...
            lblLookupDescription = new Label[4];
        ...
        void tbItemPriceInput_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                try
                {
                    ...
                    else // occurs when the input price is incorrect
                    {
                        tbItemPriceInput.Text = "";
                    }
                }
        ...
        void tbItemCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.Enter)
            {
                // Barcode entered
                //e.SuppressKeyPress = true;
                nDisplayMoveUpAmount = 0;
                tillEngine.AddItemToTransaction(tbItemCode.Text.ToUpper());
                if (tillEngine.WasItemAddSuccessful())
                {
                    int nCategory = tillEngine.GetItemJustAdded().GetItemCategory();
                    if (nCategory == 1 || nCategory == 3 || nCategory == 5)
                    {
                        lblNextItemNumber.Text = (tillEngine.GetNumberOfItemsInCurrentTransaction() + 1).ToString();
                        tbItemCode.Text = "";
                        RedrawTransactions(0);
                    ...
                    {
                        tbItemCode.Visible = false;
                        tbItemDescInput.Text = tillEngine.GetItemJustAdded().GetDescription();
                ...
                else
                {
                    ...
                    {
                        fsfiPartialBCode = new frmSearchForItem(ref tillEngine);
            ...
            else if (e.KeyCode == Keys.OemQuestion || e.KeyCode == Keys.Divide)
            {
                // Product Lookup
                tbItemCode.Visible = false;
                fsfiLookup = new frmSearchForItem(ref tillEngine);
                fsfiLookup.Show();
        ...
        void fiGetMulOrDisc_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    float nDiscountPercentage = (float)Convert.ToDecimal(sCommand[1]);
                    float fAmountBeforeDiscount = tillEngine.GetItemJustAdded().GetAmount();
                    if (TillEngine.TillEngine.FixFloatError(fAmountBeforeDiscount * (1 - (nDiscountPercentage / 100))) >= 0.01f)
                    {
                        tillEngine.DiscountPercentageFromLastItem(Convert.ToInt32(nDiscountPercentage));
                    }
                    else
                    {
                ...
                {
                    float fAmountToDiscount = TillEngine.TillEngine.fFormattedMoneyString(sCommand[1]);
                    if (fAmountToDiscount >= tillEngine.GetItemJustAdded().GetAmount())
                    {
                        AddOtherItemDescription("Discount of item " + tillEngine.GetNumberOfItemsInCurrentTransaction().ToString() + " too high. Try " + FormatMoneyForDisplay(tillEngine.GetItemJustAdded().GetAmount() - 0.01f) + " or lower|10|10|true|centre");
                    }
                    else
                    {
        ...
        void fpiGetPayment_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            else if (sPaymentType == "PDISCOUNT")
            {
                ...
                {
                    tillEngine.DiscountPercentageFromWholeTransaction(Convert.ToInt32(Math.Round(fAmountPaid, 2)));
                    AddOtherItemDescription(fAmountPaid.ToString() + "% Transaction Discount|0|0|false|rightdescalign");
                }
                else
                {
        ...
        void tbLookupStockCode_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sItemDetails != null)
                {
                    ...
                    lblLookupDescription[1].Text += tillEngine.GetCurrencySymbol().ToString() + sItemDetails[1];
                ...
                {
                    fsfiLookupLookup = new frmSearchForItem(ref tillEngine);
                ...
                {
                    lblLookupDescription[0].Text = "Barcode wasn't recognised.";
        ...
        void tbInput_KeyDown(object sender, KeyEventArgs e)
        {
            if (fsCurrentFormState == FormState.LoginScreen)
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        e.SuppressKeyPress = true;
                        int nCurrentUser = Convert.ToInt32(tbInput.Text);
                        if (nCurrentUser != 0)
                        {
                            tillEngine.ChangeStaffNumber(nCurrentUser);
                            lblStaffName.Text = "ID = " + tillEngine.GetCurrentStaffName();
                        ...
                        {
                            tillEngine.ChangeStaffNumber(-2);
                    ...
                    catch
                    {
                        ...
                        {
                            MessageBox.Show("Customer Address Entry has not yet been implemented");
                            tbInput.Text = "";
                        }
                        ...
                        else if (tbInput.Text.ToUpper() == "S")
                        {
                            tbInput.Text = "";
        ...
        void fpiChangePaymentMethod_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            {
                /*
                fasChargeToAcc = new frmAccSel(cFrmForeColour, cFrmBackColour, ref tillEngine);
                fasChargeToAcc.FormClosed += new FormClosedEventHandler(fasChargeToAcc_FormClosing);
                fasChargeToAcc.Show();
                 */
        ...
        void RedrawTransactions(int nOfItemsToMissOffBottom)
        {
            ...
            if (sNonItemDisplayArray.Length == 0)
            {
                lblCurrentTotal.Visible = true;
