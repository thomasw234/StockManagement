...
namespace TillEngine
{
    ...
    public partial class TillEngine
    {
        ...
        public void CashUp()
        {
            ...
            /*
            if (GTill.Properties.Settings.Default.bUseMSDosFileTransferProgram)
            {
                if (!File.Exists("fileList.txt"))
                    File.Create("fileList.txt");
                TextWriter writeFileList = new StreamWriter("fileList.txt");
                writeFileList.WriteLine("// Files should be in the format File1PathOnThisComputer,File1PathOnDestinationComputer;File2PathOnThisComputer,File2PathOnDestinationComputer");
                writeFileList.WriteLine("// Upto 10 files! No more!! More will cause the DOS program to crash");
                writeFileList.WriteLine(sOutGNGFolderLocation + "\\" + "REPDATA.DBF,C:\\OUTGNG\\REPDATA.DBF;" + sOutGNGFolderLocation + "\\" + "REPDATA" + DayNumber(sDateToday).ToString() + ".DBF,C:\\OUTGNG\\REPDATA" + DayNumber(sDateToday).ToString() +
                    ".DBF;" + sOutGNGFolderLocation + "\\" + "TDATA" + DayNumber(sDateToday).ToString() + ".DBF,C:\\OUTGNG\\TDATA" + DayNumber(sDateToday).ToString() + ".DBF;" + sOutGNGFolderLocation + "\\" + "THDR" + DayNumber(sDateToday).ToString() + ".DBF,C:\\OUTGNG\\THDR" + DayNumber(sDateToday).ToString() + ".DBF");
                writeFileList.Close();
            }
             */
        }
