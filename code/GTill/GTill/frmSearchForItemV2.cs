...
namespace GTill
{
     ...
     public class Item
    {
        ...
        public Item(string[] sRecordContents, decimal stockLevel)
        {
            ...
            else
            {
                ...
                fGrossAmnt = (decimal)Convert.ToDecimal(sRecordContents[4]);
    ...
    class frmSearchForItemV2 : Form
    {
        ...
        void SearchForItems()
        {
            ...
            if (!sSearchTerm.StartsWith("CAT:") && sSearchTerm != "TYPE:6")
            {
                ...
                for (int i = 0; i < nOfItems; i++)
                {
                    ...
                    iResults[i] = new Item(tEngine.GetItemRecordContents(sRecordContents[0]), (decimal)tEngine.GetItemStockLevel(sSearchResults[i, 0]));
                ...
                for (int i = 0; i < nOfItems; i++)
                {
                    ...
                    for (int x = 0; x < iResults.Length; x++)
                    {
                        ...
                        else if (iResults[x].ParentBarcode.StartsWith("@"))
                        {
                            bItemAlreadyExists[i] = true;
                            nOfItemsAlreadyExist++;
                        }
                ...
                for (int i = nPrevLength; i < iResults.Length + nOfItemsAlreadyExist; i++)
                {
                    if (!bItemAlreadyExists[i - nPrevLength])
                    {
                        ...
                        iResults[i - nOfItemsSkipped] = new Item(tEngine.GetItemRecordContents(sRecordContents[0]), (decimal)tEngine.GetItemStockLevel(sRecordContents[0]));
                ...
                for (int i = 0; i < iResults.Length; i++)
                {
                    ...
                    if (iResults[i].ParentBarcode != "")
                    {
                        ...
                        if (!bAlreadyExists && !iResults[i].ParentBarcode.StartsWith("@"))
                        {
                            ...
                            iResults[iResults.Length - 1] = new Item(tEngine.GetItemRecordContents(iResults[i].ParentBarcode), (decimal)tEngine.GetItemStockLevel(iResults[i].ParentBarcode));
                    ...
                    else
                    {
                        ...
                        if (sChildren.Length > 0)
                        {
                            for (int x = 0; x < sChildren.Length; x++)
                            {
                                ...
                                {
                                    Array.Resize<Item>(ref iResults, iResults.Length + 1);
                                    iResults[iResults.Length - 1] = new Item(tEngine.GetItemRecordContents(sChildren[x].TrimEnd(' ')), (decimal)tEngine.GetItemStockLevel(sChildren[x].TrimEnd(' ')));
                                }
                            }
                        }
                    }
            ...
            else
            {
                 ...
                 */
                string[] codes = tEngine.GetCodesOfItemsInCategory(sCategory);
                iResults = new Item[codes.Length];
                for (int i = 0; i < codes.Length; i++)
                {
                    iResults[i] = new Item(tEngine.GetItemRecordContents(codes[i]), (decimal)tEngine.GetItemStockLevel(codes[i]));
                }
            }
            OrderResults();
            CountNumberOfResultsToShow();
        ...
        private string FormatMoneyForDisplay(decimal dAmount)
        {
        ...
        private string FormatMoneyForDisplay(string sAmount)
        {
            try
            {
