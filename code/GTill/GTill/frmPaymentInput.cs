...
namespace GTill
{
    class frmPaymentInput : Form
    {
        ...
        public frmPaymentInput(Point pStartLocation, Size sFormSize, string[] sCCTypes, float fFullAmountDue, bool bChangeAllowedFromCheque, bool bAllowDisc, bool bChangePayment)
        {
            ...
            this.KeyUp += new KeyEventHandler(frmPaymentInput_KeyUp);
            SetupForGettingAmount();
            ListCreditCards();
           
        }

        void frmPaymentInput_KeyUp(object sender, KeyEventArgs e)
        {
            if (!bKeyUp)
            {
                SetupFromCards();
        ...
        // Check if subtract has been released
        bool bKeyUp = false;
                ...
                else
                {
                    try
                    {
                        ...
                        if (nKeyNumber > 0 && nKeyNumber <= lblCreditCardChoices.Length)
                        {
                            ...
                            if (!bKeyUp)
                            {
                                sPaymentType = "CRCD" + (e.KeyCode.ToString()[e.KeyCode.ToString().Length - 1]);
                                fAmountInput = fAmountRemainingDue;
                            }
                            this.Close();
                        }
                    }
        ...
        void fiDiscountAmnt_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            else
            {
                ...
                try
                {
                    if (sPaymentType == "ADISCOUNT"|| sPaymentType == "SDISCOUNT")
        ...
        {
            if (e.KeyCode == Keys.Enter || (bJumpedStraightToCash == true && (e.KeyCode == Keys.Multiply || e.KeyCode == Keys.PageUp)))
            {
                try
                {
                    fAmountInput = TillEngine.TillEngine.fFormattedMoneyString(tbGetAmount.Text);
        ...
        public void SelectCard(int nCardNum)
        {
            sPaymentType = "CRCD" + nCardNum.ToString();
            fAmountInput = fAmountRemainingDue;
            this.Close();
        }
